...
def chaffy_mask(salt, LTC, m, n):
    ...
    r = out_1 ^ out_2
    return np.reshape(out_1 ^ out_2, (n, ))
...
# solve()
salt = b'a'*(m//8) + b'b'*(m//8)
# io.sendlineafter(b"Give me your salt: ", salt)
# io.interactive()
chaffy = chaffy_mask(salt, gen_LTC(), m, n)
print(chaffy)
masked_flag = '2a202f66d560d5831f1a0550f14ed5840e17395dcf5fdca8081c0a59c75fdd98052c0f41f15bd5843400095ac143db98041c095ac143db98042c0354dd75958a'
masked_flag = [int(masked_flag[i:i+2], 16) for i in range(0, len(masked_flag), 2)]
masked_flag = np.array(masked_flag, dtype=(int))
flag = chaffy ^ masked_flag 
flag %= 256
print(flag)
print(bytes(list(flag)).decode())
